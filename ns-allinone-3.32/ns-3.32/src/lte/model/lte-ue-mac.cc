...
LteUeMac::RandomlySelectAndSendRaPreambleNb ()
{
    ...
    if (startSubframeNprachOccasion != currentsubframe)
    {
        ...
        NS_BUILD_DEBUG (std::cout << " SendRaPreambleNb "<<" imsi " << m_imsi<< std::endl);
...
LteUeMac::SendRaPreambleNb (bool contention)
{
    ...
    NS_LOG_FUNCTION (this << " inside SendRaPreambleNb RNTI to be assigned: "<<(uint32_t) m_raRnti<< " IMSI: "<< m_imsi);
...
LteUeMac::RecvRaResponse (BuildRarListElement_s raResponse)
{
    ...
    if (m_rnti == 0) {
        ...
        NS_LOG_INFO ("got RAR for RAPID " << (uint32_t) m_raPreambleId
                                          << " but not setting "
                                          << " T-C-RNTI = " << raResponse.m_rnti
                                          << " as we have previous non zero RNTI "<< m_rnti
        );
...
LteUeMac::RecvRaResponseNb (NbIotRrcSap::RarPayload raResponse) {
    ...
    if (m_rnti == 0) {
        ...
        if (m_mac_logging) {
            ...
            std::ofstream logfile;
            logfile.open(logfile_path, std::ios_base::app);
            logfile << m_imsi << ",RecvRaResponseNb," << Simulator::Now().GetMilliSeconds() << ",cellRNTI,"
                    << raResponse.cellRnti << "\n";
            logfile.close();
        }
        ...
        if ((lc0BsrIt != m_ulBsrReceived.end()) && (lc0BsrIt->second.txQueueSize > 0)) {
            ...
            uint32_t subframesTillNpusch =
                    raResponse.ulGrant.subframes.second.front() - (10 * (m_frameNo - 1) + m_subframeNo - 1);

            m_transmissionScheduled = true;
            Simulator::Schedule(MilliSeconds(subframesTillNpusch), &LteUeCmacSapUser::NotifyEnergyState, m_cmacSapUser,
            ...
            Simulator::Schedule(MilliSeconds(subframes + 1), &LteUeCmacSapUser::NotifyEnergyState, m_cmacSapUser,
...
LteUeMac::RaResponseTimeout (bool contention)
{
    ...
    {
        NS_LOG_DEBUG ("RAR timeout, preambleTransMax reached => giving up : " << m_imsi<<" counter: "<<m_rachConfig.preambleTransMax);
        m_cmacSapUser->NotifyRandomAccessFailed ();
    }
    else
    {
        NS_LOG_DEBUG ("RAR timeout, re-send preamble");
        if (contention)
        {
...
LteUeMac::RaResponseTimeoutNb (bool contention)
{
    ...
    m_raResponseTimeoutTrace (m_imsi, contention, m_preambleTransmissionCounter,
                              m_rachConfig.preambleTransMax + 1);
    if (m_preambleTransmissionCounter == 10000)
    ...
    {
        NS_LOG_DEBUG ("RAR timeout, re-send preamble for imsi: " << m_imsi );
        if (contention)
        {
...
LteUeMac::DoStartRandomAccessProcedureNb (bool edt)
{
    ...
    NS_BUILD_DEBUG (std::cout << " DoStartRandomAccessProcedureNb "<<" imsi " << m_imsi<< std::endl);
    ...
    {
        // CE2
        NS_BUILD_DEBUG (std::cout << "CE LEVEL 2 " << std::endl);
    ...
    {
        // CE1
        NS_BUILD_DEBUG (std::cout << "CE LEVEL 1 " << std::endl);
    ...
    {
        // CE0
        NS_BUILD_DEBUG (std::cout << "CE LEVEL 0 " << std::endl);
