           ...
           iter != receivedNprachs.end (); ++iter)
        {
          if (iter->second == 1)
          ...
          else if (iter->second > 1)
            {
             ...
             // NS_LOG_INFO(std::cout << "==================================================" <<  std::endl);
//              NS_LOG_INFO ( "Collision" );
              std::cout << "collision "<< Simulator::Now().GetMilliSeconds() <<  std::endl;
                // NS_LOG_INFO(std::cout << "==================================================" <<  std::endl);
              ...
              if (m_dropPreambleCollision)
              ...
              else
                {
                  ...
                  //NS_LOG_INFO (std::cout << "Preamble received of offset " << int (subcarrierOffset) << " at Subframe " << (10 * (m_frameNo - 1) + (m_subframeNo - 1)) << std::endl);
                  NbIotRrcSap::Rar rar;
                  rar.cellRnti = m_cmacSapUser->AllocateTemporaryCellRnti ();
                  rar.rapId = subcarrierOffset + iter->first;
                  rar.rarPayload.cellRnti = rar.cellRnti;
