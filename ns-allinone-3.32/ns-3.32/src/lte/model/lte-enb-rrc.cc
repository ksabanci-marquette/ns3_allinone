...
{
    NS_LOG_FUNCTION(this << " m_componentCarrierId "<< m_componentCarrierId);
...
LteEnbRrc::SendData (Ptr<Packet> packet)
{
  ...
  NS_ASSERT_MSG (found, "no EpsBearerTag found in packet to be sent");
    ...
    std::cout << "LteEnbRrc::SendData imsi: " << +tag.GetImsi() << "," << Simulator::Now().GetNanoSeconds() << std::endl;
...
{
    //burada
    // yeni rnti atamadan once UeManager icinde 0 dan farkli RNTI no varmi diye bakilabilir

  NS_LOG_FUNCTION (this << " componentCarrierId " << componentCarrierId);
  bool found = false;
  uint16_t rnti;
...
LteEnbRrc::ResumeUe(uint16_t rnti, uint64_t resumeId){
    NS_LOG_FUNCTION (this <<"rnti resumeid"<< (uint16_t) rnti <<" "<< (uint64_t) resumeId );
