...
{
  NS_LOG_FUNCTION (this << " "<< packet->GetSize() << " "<< Simulator::Now().As(Time::S));
  ...
  if (drbid != 0)
    {
      ...
      std::cout << "LteUeRrc::SendDataNb imsi: " << m_imsi << "," << Simulator::Now().GetNanoSeconds() << std::endl;
...
{
//    burda  -- baglanamayan cihazlar hep buraya dusuyor: "UeRrc IDLE_WAIT_MIB --> IDLE_RANDOM_ACCESS"
  NS_LOG_FUNCTION (this << m_imsi << ToString (m_state));
