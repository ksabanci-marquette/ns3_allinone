...
void TcpEchoClient::SetPacketSizes(uint32_t * sizes){
  m_PacketSizes = sizes;
}


uint32_t * TcpEchoClient::GetPacketSizes(){

  return m_PacketSizes;
}

void TcpEchoClient::PrintPacketSizes(uint32_t * arr, int length){
...
void TcpEchoClient::deleteFirst(uint32_t * arr, int length, uint32_t * retArr){
...
uint32_t TcpEchoClient::returnElement(uint32_t *arr, int length, int index){
  for (int i=0;i<length;i++){
      if(i==index){
          return arr[index];
        }
