...
public:
  ...
  void PrintPacketSizes(uint32_t * arr, int length);

  void deleteFirst(uint32_t * arr, int length, uint32_t * retArr);

  uint32_t returnElement(uint32_t *arr, int length, int index);
