...
TcpEchoServer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TcpEchoServer")
    ...
    .AddAttribute ("Local", "The Address on which to Bind the rx socket.",
                   AddressValue (),
                   MakeAddressAccessor (&TcpEchoServer::m_local),
                   MakeAddressChecker ())
...
void TcpEchoServer::SetPacketSizes(uint32_t * sizes){
  m_PacketSizes = sizes;
}


uint32_t * TcpEchoServer::GetPacketSizes(){

  return m_PacketSizes;
}


uint32_t TcpEchoServer::returnElement(uint32_t *arr, int length, int index){
  ...
  if (m_socket == 0)
	{
		...
		m_socket = Socket::CreateSocket (GetNode (), tid);
		 NS_LOG_INFO("Echo Server local address:  " << Ipv4Address::ConvertFrom (m_local) << " port: " << m_port  );
...
void TcpEchoServer::HandleAccept (Ptr<Socket> s, const Address& from)
{
	...
	new_socket->Connect (InetSocketAddress (Ipv4Address::ConvertFrom(dst), m_port));
	...
	while (packet = socket->RecvFrom (from))
	{
		...
		if (InetSocketAddress::IsMatchingType (from))
		{
			...
			//if (mtag_old.GetSrcNode() == GetNode()->GetId()) 
				...
				//dst = m_local;
                                dst = InetSocketAddress::ConvertFrom(m_local).GetIpv4 ();
		/*		mtag_new.SetMalStatus(MaliciousTag::FROMPROXY);
